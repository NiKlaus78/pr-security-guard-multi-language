...
Supports multi-language repos: Java (pom.xml), JavaScript/Node.js (package.json),
...
class ScanRequest(BaseModel):
    ...
    package_json_content: str = ""          # npm (JavaScript/Node.js)
...
app = FastAPI(
    ...
    description="LangGraph-powered multi-language security analysis pipeline",
    version="2.0.0",
    lifespan=lifespan
)
...
async def scan_pr(request: ScanRequest):
    ...
    if not request.diff_content.strip():
        return ScanResponse(
            scan_id=scan_id, langsmith_run_id="", findings=[],
            gate_decision="ALLOW", summary="No diff content to scan.", duration_ms=0
        )
