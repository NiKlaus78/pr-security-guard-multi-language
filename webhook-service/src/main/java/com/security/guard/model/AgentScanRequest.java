 ...
 * Supports multi-language dependency manifests:
 *   pom.xml          -> Maven (Java)
...
public class AgentScanRequest {
    ...
    private String pomXmlContent;              // Full pom.xml when changed in PR

    @JsonProperty("package_json_content")
    private String packageJsonContent;         // Full package.json when changed in PR

    @JsonProperty("requirements_txt_content")
    private String requirementsTxtContent;     // Full requirements.txt when changed in PR
}
